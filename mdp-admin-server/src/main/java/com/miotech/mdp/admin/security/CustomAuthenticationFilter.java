...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (authentication != null && StringUtils.isNotEmpty(authentication.getName()) && !StringUtils.equals(authentication.getName(), "anonymousUser")) {
            UserInfo savedUser;
            if (securityService.isNewUser(authentication.getName())) {
                UserInfo savingUser = new UserInfo();
                savingUser.setUsername(authentication.getName());
                savedUser = securityService.saveUser(savingUser);
            } else {
                savedUser = securityService.getUser(authentication.getName());
            }
